...
public class TerminalPanel extends JComponent implements TerminalDisplay, TerminalActionProvider {
	...
	void setUpClipboard() {
		myClipboard = Toolkit.getDefaultToolkit().getSystemClipboard(); // system
																		// wide
		// if (myClipboard == null) {
	...
	protected void copySelection(final Point selectionStart, final Point selectionEnd) {
		...
		final String selectionText = SelectionUtil.getSelectionText(selectionStart, selectionEnd, myTerminalTextBuffer);
	...
	public void paintComponent(final Graphics g) {
		...
		myTerminalTextBuffer.processHistoryAndScreenLines(myClientScrollOrigin, new StyledTextConsumer() {
			...
			public void consume(int x, int y, @NotNull TextStyle style, @NotNull CharBuffer characters, int startRow) {
				...
				if (mySelection != null) {
					Pair<Integer, Integer> interval = mySelection.intersect(x, row + myClientScrollOrigin,
							characters.length());
					if (interval != null) {
						TextStyle selectionStyle = getSelectionStyle(style);
						drawCharacters(interval.first, row, selectionStyle,
								characters.subBuffer(interval.first - x, interval.second), gfx);
					}
				}
			}
			...
			public void consumeNul(int x, int y, int nulIndex, TextStyle style, CharBuffer characters, int startRow) {
				...
				if (mySelection != null) {
					...
					Pair<Integer, Integer> interval = mySelection.intersect(nulIndex, row + myClientScrollOrigin,
			...
			public void consumeQueue(int x, int y, int nulIndex, int startRow) {
				if (x < columnCount) {
					consumeNul(x, y, nulIndex, TextStyle.EMPTY,
							new CharBuffer(CharacterUtils.EMPTY_CHAR, columnCount - x), startRow);
				}
			}
		});
	...
	private void drawInputMethodUncommitedChars(Graphics2D gfx) {
		if (myInputMethodUncommitedChars != null && myInputMethodUncommitedChars.length() > 0) {
			...
			BasicStroke dotted = new BasicStroke(1, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0,
					new float[] { 0, 2, 0, 2 }, 0);
	...
	protected void drawImage(Graphics2D g, BufferedImage image, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1,
			int sx2, int sy2) {
	...
	private void drawCharacters(int x, int y, TextStyle style, CharBuffer buf, Graphics2D gfx) {
		...
		gfx.fillRect(xCoord, yCoord, Math.min(textLength * myCharSize.width, getWidth() - xCoord),
	...
	private void drawChars(int x, int y, CharBuffer buf, TextStyle style, Graphics2D gfx) {
		...
		while (offset + newBlockLen <= buf.length()) {
			...
			// while (myMonospaced && (offset + newBlockLen < buf.getLength())
			// && isWordCharacter(buf.charAt(offset + newBlockLen - 1))
			// && (font == getFontToDisplay(buf.charAt(offset + newBlockLen -
			// 1), style))) {
			// newBlockLen++;
			// }
			gfx.setFont(font);
...
//		gfx.setClip(null);
	...
	private void updateScrolling() {
		if (myScrollingEnabled) {
			myBoundedRangeModel.setRangeProperties(0, myTermSize.height,
					-myTerminalTextBuffer.getHistoryBuffer().getLineCount(), myTermSize.height, false);
		} else {
	...
	private static boolean isCodeThatScrolls(int keycode) {
		return keycode == KeyEvent.VK_UP || keycode == KeyEvent.VK_DOWN || keycode == KeyEvent.VK_LEFT
				|| keycode == KeyEvent.VK_RIGHT || keycode == KeyEvent.VK_BACK_SPACE || keycode == KeyEvent.VK_DELETE;
	}
	 ...
	 * http://docs.oracle.com/javase/7/docs/technotes/guides/imf/api-tutorial.
	 * html
	 */
	@Override
	...
	private class MyInputMethodRequests implements InputMethodRequests {
		...
		public Rectangle getTextLocation(TextHitInfo offset) {
			Rectangle r = new Rectangle(myCursor.getCoordX() * myCharSize.width,
